...
Notifications: a `notifications` outbox, one rendered row per recipient per event written in the same transaction as the event, which the `notify` edge function drains when the database wakes it ([ADR-0008](adr/0008-notification-outbox.md)); the notification matrix below is stored on each row as its channels.
